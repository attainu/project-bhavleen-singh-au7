...
const MainCards = ({
    ...
    auth,
    post,
    post: { _id, image, caption, owner, likes, comments, createdAt },
}) => {
        ...
        <Fragment>
            <Card className={classes.root} elevation={4}>
                <CardHeader
                    ...
                    title={<Link to={`/open/profile/${owner._id}`}><strong>{owner.name}</strong></Link>}
                    subheader={owner.username}
                />
                <CardMedia className={classes.media} image={image.imageUrl} />
                <CardContent>
                    <Typography
                        variant="body2"
                        color="textPrimary"
